...
Realize the tier bindings by specifying the exact model and reasoning effort parameter for each agent in the orchestration prompt or the subagent's configuration. Wherever the runtime does not support the requested reasoning effort or model, state what was used instead. Persona, scoping, and evidence rules apply regardless of the actual running model.

## Route to project agents

Read the persona file for the assigned role from `agents/<agent-name>.md` next to this SKILL.md and inject it verbatim at the top of the delegated prompt.
