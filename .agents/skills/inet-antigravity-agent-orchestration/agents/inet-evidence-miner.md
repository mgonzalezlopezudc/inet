...
- Tier: Luna-tier — Gemini 3.5 Flash (`gemini-3.5-flash`), effort `low`
- Scope: read-only
- Use for bounded INET searches, artifact inventories, and exact filtering of existing logs, captures, event logs, scalars, or vectors. Use only when the question and output schema are explicit.
