...
For any assignment that proposes, scopes, or audits a change under src/inet, use inet-architectural-requirements. Before recommending a change surface, enumerate the affected paths, resolve their seal status, map the relevant R-* and AR-* identifiers, consult naming rules and both exception ledgers when applicable, and identify the appropriate focused check-architecture.sh scope. If the scope touches `src/inet/linklayer/ieee80211/` or `src/inet/physicallayer/wireless/ieee80211/`, read `.agents/skills/inet-architectural-requirements/references/ieee80211-architectural-requirements.md` in full and include every applicable `AR-WLAN-*` identifier in the map. Return this as pre-change evidence for the parent, implementer, or reviewer; do not issue the final compliance verdict, authorize a sealed-file edit, or propose a ledger or sealing-status edit as already approved.
