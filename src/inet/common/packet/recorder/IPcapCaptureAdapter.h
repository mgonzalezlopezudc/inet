...
class INET_API PcapCaptureRecord
{
  ...
  public:
    const b frontOffset;
    const b backOffset;
    const bool aggregateSubframe;

    PcapCaptureRecord(b frontOffset, b backOffset, std::vector<uint8_t> prefix = {}, bool aggregateSubframe = false) :
        prefix(std::move(prefix)), frontOffset(frontOffset), backOffset(backOffset), aggregateSubframe(aggregateSubframe) {}
