    ...
    const ITransmissionSpectrum *dataSpectrum, int minimumIntervals)
{
    ...
    // Infer the smallest regular occupied-carrier spacing from immutable
    // spectrum metadata. This deliberately contains no IEEE-specific values;
    // a regular plan such as HT20/HT40 therefore gets a finer grid only when
    // the delay-derived grid would otherwise miss a carrier center.
    ...
    // A partitioned occupied-carrier band is bounded half a carrier spacing
    // away from its center.  An even number of carrier-spacing multiples
    // therefore places both the center and the band edge on the evaluation
    // grid.  This also avoids producing a one-sided interval when an
    // adaptive-grid partition ends at an occupied-band boundary.
...
int inferCarrierAlignedFrequencyIntervals(const DimensionalSignalAnalogModel *analogModel,
    const ITransmission *transmission, int minimumIntervals)
{
    const auto partitioned = dynamic_cast<const IPartitionedTransmissionSpectrum *>(transmission->getAnalogModel());
    const auto dataSpectrum = partitioned == nullptr ? nullptr : partitioned->getDataSpectrum();
    return DimensionalMediumAnalogModel::computeCarrierAlignedFrequencyIntervals(analogModel->getCenterFrequency(),
        analogModel->getBandwidth(), dataSpectrum, minimumIntervals);
}
    ...
    const std::shared_ptr<const IChannelMatrixSnapshot>& snapshot) const
{
    ...
    const int minimumFrequencyIntervals = maximumDelaySeconds == 0 ? 2 :
        std::max(2, (int)std::ceil(bandwidthHz * 20 * maximumDelaySeconds));
    const int frequencyIntervals = inferCarrierAlignedFrequencyIntervals(analogModel, transmission, minimumFrequencyIntervals);
