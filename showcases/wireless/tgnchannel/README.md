...
variation enabled in the base configuration. `ModelA` through `ModelF` select
the exact Appendix C profiles; `Siso`, the default 2×1 case, and `Matrix2x2`
exercise the supported matrix dimensions. `StaticDiagnostic` is intentionally
only a deterministic diagnostic fixture.
