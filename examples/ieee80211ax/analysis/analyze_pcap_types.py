#!/usr/bin/env python3
"""
IEEE 802.11 Packet Type Statistics and Airtime Analysis Script

This compatibility entry point defaults to the IEEE 802.11ax suite. The
generation-neutral launcher configures it from a suite descriptor so the same
capture, typed-PHY, evidence, and walkthrough machinery can analyze other
...
from inet_wifi_analysis import decode_phy_observation, extract_eht_radiotap
from analysis_core import SESSION_ID_PATTERN, atomic_write_text

# Resolve project repository root (3 levels up from this file)
REPOSITORY_ROOT = Path(__file__).resolve().parents[3]
EXAMPLE_ROOT = REPOSITORY_ROOT / "examples" / "ieee80211ax"
ANALYSIS_OUTPUT_DIR = Path(__file__).resolve().parent
MANIFEST_PATH = ANALYSIS_OUTPUT_DIR / "pcap_statistics_manifest.json"
MANIFEST_HISTORY_DIR = ANALYSIS_OUTPUT_DIR / "pcapmanifests"
GENERATED_MARKER = "ieee80211ax-pcap-statistics"
...
subdirs_configs = {
    ...
    "dl_ofdma": [
        ...
        "BacklogBased4ms", "HoLMinDelay4ms",
        "BacklogBased3ms", "HoLMinDelay3ms",
        "BacklogBased3_5ms", "HoLMinDelay3_5ms",
        "BacklogBased2_5ms", "HoLMinDelay2_5ms",
    ...
    "dense_iot": ["AxUl", "AcUl", "AxDl", "AcDl", "AxMixed", "AcMixed"],
    # UoraHeavyContention is intentionally omitted until its long-running
    # packet-capture validation is completed. Its scalar/vector campaign
    # remains part of the result analysis.
    "ul_ofdma": ["ScheduledOnly", "MixedUora", "EqualRus", "EdcaBaseline",
                 "UoraLightContention", "UoraMoreRandomAccessRus"],
    "he_er_su": ["ErBss", "CellBoundaryHeSu", "CellBoundaryHeErSu"]
}


def configure_suite(suite, output_dir):
    """Configure this compatibility entry point from a neutral suite."""
...
def capture_source_state():
    ...
    output_relative = str(ANALYSIS_OUTPUT_DIR.relative_to(REPOSITORY_ROOT))
    diff_command = [
        "git", "diff", "--binary", "HEAD", "--", "src", example_relative,
        f":(exclude){example_relative}/**/walkthrough.md",
        f":(exclude){example_relative}/**/*.png",
        f":(exclude){output_relative}/summary_results_pcap.json",
        f":(exclude){output_relative}/pcap_statistics_manifest.json",
        f":(exclude){output_relative}/pcapmanifests/**",
    ]
    diff = subprocess.run(diff_command, cwd=str(REPOSITORY_ROOT), check=True,
                          stdout=subprocess.PIPE, stderr=subprocess.PIPE).stdout
    untracked = command_output([
        "git", "ls-files", "--others", "--exclude-standard", "--",
        "src", example_relative,
    ]).splitlines()
    excluded_paths = {
        f"{output_relative}/summary_results_pcap.json",
        f"{output_relative}/pcap_statistics_manifest.json",
    }
    digest = hashlib.sha256(diff)
    for relative in sorted(untracked):
        if (
            relative in excluded_paths
            or relative.endswith("/walkthrough.md")
            or relative.endswith(".png")
            or relative.startswith(f"{output_relative}/pcapmanifests/")
        ):
            continue
