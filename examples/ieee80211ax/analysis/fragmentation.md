...
In the refreshed five-seed campaign, dynamic and static policies both have a
mean transmitted MAC-frame size of `293.89 B` (95% CI `±0.88 B`) and total ACK
airtime of `51.792 ms` (95% CI `±0.332 ms`). The unfragmented control is
`1070 B` and `69.360 ms`, respectively. These values come from the HCF and
radio vectors in the `0.3–2.0 s` window.
