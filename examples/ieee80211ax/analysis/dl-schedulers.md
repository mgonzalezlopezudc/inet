...
The top row is a symmetric three-flow comparison of SU EDCA, equal-RU first-backlogged-work, and equal-RU first-head-of-line policies. The bottom row isolates backlog-based versus minimum-HoL-delay scheduling under 4, 1.6, and 0.4 Mbit/s offered flows. Goodput and packet-delay p95 are computed once per run. Symmetric fairness uses station goodputs directly; asymmetric fairness first divides each station's goodput by its offered rate so a scheduler is not penalized for respecting unequal demand.
...
averages `5.979 Mbps` with p95 delay `3.39 ms`, while HoL min delay averages
`5.926 Mbps` with p95 delay `7.99 ms`; these values illustrate why the
